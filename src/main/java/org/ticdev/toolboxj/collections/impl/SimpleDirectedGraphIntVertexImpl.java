 ...
 *
 * <p>
 * The implementation uses an adjacency matrix. The meaning of source
 * node and destination node is strict. New nodes can be added to the
 * graph, but nodes cannot be removed. Each vertex is represented as an
 * {@link Integer} that represents its position in the adjacency matrix.
 * A null value for an edge from one node to another means that there
 * is no direct path from the source node to the destination node.
 * </p>
 *
 * @param <EDGE> the edge type
 * @author <a href="mailto:tandauioan@gmail.com">Ioan - Ciprian Tandau</a>
 */
public final class SimpleDirectedGraphIntVertexImpl<EDGE>
    implements Graph<Integer, EDGE> {
  ...
  /**
   * Sets and edge from the source vertex to the destination vertex.
   * If the edge is null, then there will be no edge between the source
   * and the destination (existing edge, if any, will be removed).
   *
  ...
  @Override
  public boolean hasVertex(final Integer vertex) {
    return vertex != null && vertex >= 0 && vertex < vertices.size();
  }

  @Override
  public void assertHasVertex(final Integer vertex)
      throws IllegalArgumentException {
    if (!hasVertex(vertex)) {
  ...
  @Override
  public Set<Integer> reachableFrom(
      final Integer vertex,
      final boolean excludeSelf) {
    assertHasVertex(vertex);
    final Set<Integer> result = new HashSet<>();
  ...
  public boolean edgeExists(
      final Integer vertex1,
      final Integer vertex2) {
