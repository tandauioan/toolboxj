...
public interface GraphSupport {
   ...
   * @param zeroValue             the zero value for the addition operation.
   *                              The distance from a node to itself is
   *                              zero.
   * @param infinityValue         the value to use as infinite, a
   *                              substitute for no path between two
   *                              vertices
   * @param path                  the list that will contain the nodes
   *                              for the shortest path
   * @return the distance, possible infinite if there's
   *     no path between the start and destination vertices
   * @throws Exception if an exception occurs
   */
  static <VERTEX, EDGE, WEIGHT> WEIGHT shortestPath(
      final Graph<VERTEX, EDGE> graph,
      final VERTEX start,
      final VERTEX destination,
      final UnaryFunction<EDGE, WEIGHT> edgeToWeight,
      final ReduceFunction<WEIGHT> additiveWeightReducer,
      final Comparator<WEIGHT> weightComparator,
      final WEIGHT zeroValue,
      final WEIGHT infinityValue,
      final Deque<VERTEX> path) throws Exception {
    ...
    while (!all.isEmpty()) {
      ...
      if (weightComparator.compare(
          distances.get(current), infinityValue) == 0) {
        /*
    ...
    if (weightComparator
        .compare(distances.get(current), infinityValue) == 0) {
