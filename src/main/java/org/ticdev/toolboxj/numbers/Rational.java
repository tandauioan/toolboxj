 ...
 * <p>
 * For methods which return a rational instance, whether
 * or not the result is a new instance or an existing instance
 * (including this) is implementation defined.
 * </p>
 * <p>
 * The {@link Object#equals(Object)} and {@link Object#hashCode()}
 * methods must be implemented to match the {@link PairView} class.
 * A direct result of this is that {@link Object#equals} and
 * {@link Rational#compareTo(Object)}
 * may not return consistent values for equality, but consistent
 * with the {@link java.math.BigDecimal} behavior.
 * For example, given 1/3 and 2/6,
 * an implementation may return false for equality, but it must return
 * 0 for compareTo. Implementations are not required to reduce fractions,
...
public interface Rational<ARGS, R extends Rational<ARGS, R>>
  ...
  /**
   * Returns true if this instance does not represent a valid rational
   * number. For example, if both the numerator and denominator are 0,
   * the division results in non-a-number.
   *
   * @return true if this instance does not represent a valid
   *     rational number.
   */
  boolean isNaN();

  /**
   * Returns true if this instance represents a value of 0. For example
   * if the numerator is 0 and the denominator is a non-zero value.
   *
  ...
  /**
   * Returns true if this instance represents positive infinity.
   * For example, if the numerator is a positive number and the
   * denominator is 0.
   *
  ...
  /**
   * Returns true if this instance represents negative infinity.
   * For example, if the numerator is a negative number and the
   * denominator is 0.
   *
  ...
  /**
   * Returns the result of multiplying this rational
   * by the given rational.
   *
